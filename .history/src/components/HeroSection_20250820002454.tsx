...
const HeroSection: React.FC = () => {
  ...
  const handleProcessClick = (action: string) => {
    if (!selectedFile) {
      window.alert("Please upload a PDF first.");
      return;
    }
    // Show preview only (no download for unregistered)
    if (!previewUrl) {
      const url = URL.createObjectURL(selectedFile);
      setPreviewUrl(url);
    }
    // TODO: Implement actual processing logic and Cloudflare R2 upload
  };
  return (
                ...
                <div className="space-y-6">
                  ...
                  {previewUrl && (
                    <div className="mt-6">
                      ...
                      />
                      <div className="text-xs text-gray-500 mt-2">Preview only. Download available for registered users.</div>
                    </div>
                  )}
