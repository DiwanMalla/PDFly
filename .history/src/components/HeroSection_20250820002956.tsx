...
const HeroSection: React.FC = () => {
  ...
  const [processedFile, setProcessedFile] = useState<{ key: string; action: string } | null>(null);
  ...
  const handleProcessClick = async (action: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        window.alert(`PDF ${action.toLowerCase()} completed! Preview available below. Sign up to download processed file.`);
      }, 2000);

    } catch (error) {
      console.error('Error processing PDF:', error);
      window.alert('Error processing PDF. Please try again.');
      setIsProcessing(false);
    }
  };
  return (
                  ...
                  {previewUrl && (
                    <div className="mt-6">
                      ...
                      <div className="text-xs text-gray-500 mt-2">
                        Preview only. {processedFile ? `${processedFile.action} completed! ` : ''}Download available for registered users.
                      </div>
                      {processedFile && (
                        <div className="mt-2">
                          <button className="text-blue-600 text-sm hover:underline" onClick={() => window.location.href = '/signup'}>
