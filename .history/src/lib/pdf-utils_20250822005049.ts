...
export const loadPDFPages = async (file: File): Promise<PDFPageInfo[]> => {
  try {
    ...
    // Use the web build to avoid Node.js dependencies
    const pdfjsLib = await import("pdfjs-dist/web/pdf_viewer");
    const { getDocument, GlobalWorkerOptions } = await import("pdfjs-dist");

    // Set up the worker only in browser environment
    if (typeof window !== "undefined" && typeof document !== "undefined") {
      // Use the exact version we installed with a reliable CDN
