...
const SplitPage: React.FC = () => {
  ...
  const handleSplit = async () => {
    ...
    sessionStorage.setItem('splitConfig', JSON.stringify({
      fileName: splitConfig.fileName,
      options: splitConfig.options,
      selectedPages: splitConfig.pages.filter(p => p.selected).map(p => p.pageNumber),
      totalPages: splitConfig.pages.length,
    }));
                    ...
                    {splitOptions.mode === "ranges" && (
                      ...
                      >
                        ...
                        <input
                          ...
                          className="w-full p-2 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white text-sm"
