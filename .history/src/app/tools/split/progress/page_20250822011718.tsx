  ...
  useEffect(() => {
    const processSplit = async () => {
      try {
        ...
        const splitFileData = sessionStorage.getItem('splitFileData');

        if (!splitConfigStr || !splitFileData) {
        ...
        const fileDataArray = JSON.parse(splitFileData);
        const uint8Array = new Uint8Array(fileDataArray);
        const blob = new Blob([uint8Array], { type: 'application/pdf' });
        const file = new File([blob], splitConfig.fileName, { type: 'application/pdf' });
        ...
        switch (splitConfig.options.mode) {
          case "pages":
            if (splitConfig.selectedPages.length === 0) {
              throw new Error("No pages selected for splitting");
            }
            pagesToSplit = splitConfig.selectedPages.map((page: number) => [page]);
            break;

          case "ranges":
            if (splitConfig.options.ranges.trim()) {
              const ranges = splitConfig.options.ranges.split(",").map((range: string) => {
          ...
          case "every":
            const n = splitConfig.options.everyNPages;
            for (let i = 0; i < splitConfig.totalPages; i += n) {
              const chunk = [];
              for (let j = i; j < Math.min(i + n, splitConfig.totalPages); j++) {
                chunk.push(j + 1);
              }
        ...
        for (let i = 0; i < pagesToSplit.length; i++) {
          ...
          setSplitProgress((prev) => ({
            ...
            currentStep: `Processing split ${i + 1} of ${pagesToSplit.length}...`,
            speed: "3.2 MB/s",
          }));
          ...
          const fileName =
            pages.length === 1
              ? `${splitConfig.fileName.replace('.pdf', '')}_page_${pages[0]}.pdf`
              : `${splitConfig.fileName.replace('.pdf', '')}_pages_${pages[0]}-${pages[pages.length - 1]}.pdf`;
        ...
        setTimeout(() => {
          ...
          sessionStorage.removeItem('splitFileData');
        }, 500);
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-4xl font-bold mb-2">
                    {splitProgress.isProcessing ? "Splitting PDF..." : "Split Complete!"}
                  </h1>
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Continue Processing</h3>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    >
                      ...
                      <div className="font-medium text-gray-900">Merge Files</div>
                    ...
                    >
                      ...
                      <div className="font-medium text-gray-900">Split Again</div>
                      <div className="text-xs text-gray-500">New split task</div>
                    </button>
                  </div>
                </div>
              </div>
