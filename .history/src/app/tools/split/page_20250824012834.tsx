...
const SplitPage: React.FC = () => {
  ...
  // Continue Processing: Save split file info and redirect
  const continueProcessing = (result: SplitResult, tool: "merge" | "compress" | "convert") => {
    // Save split file info in sessionStorage
    const splitProcessConfig = {
      fileName: result.name,
      pages: result.pages,
      preview: result.preview,
      size: result.size,
      blob: result.blob, // For unsigned users, keep in browser
    };
    sessionStorage.setItem("continueProcessConfig", JSON.stringify(splitProcessConfig));
    // Redirect to selected tool
                        ...
                        {splitResults.map((result, index) => (
                          ...
                          >
                            ...
                            <div className="flex space-x-1">
                              ...
                              <button
                                onClick={() => continueProcessing(result, "merge")}
                                className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors text-xs font-semibold"
                                title="Continue to Merge"
                              >
                                Merge
                              </button>
                              <button
                                onClick={() => continueProcessing(result, "compress")}
                                className="p-1.5 text-pink-600 hover:bg-pink-50 rounded-md transition-colors text-xs font-semibold"
                                title="Continue to Compress"
                              >
                                Compress
                              </button>
                              <button
                                onClick={() => continueProcessing(result, "convert")}
                                className="p-1.5 text-orange-600 hover:bg-orange-50 rounded-md transition-colors text-xs font-semibold"
                                title="Continue to Convert"
                              >
                                Convert
                              </button>
                            </div>
