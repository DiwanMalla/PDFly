...
const MergePage: React.FC = () => {
  ...
  // Get initial file(s) from other tools
  useEffect(() => {
    const loadInitialFiles = () => {
      // Check for multiple split files first
      const splitFilesMeta = sessionStorage.getItem("splitFilesMeta");
      const splitFilesBlobs = sessionStorage.getItem("splitFilesBlobs");

      if (splitFilesMeta && splitFilesBlobs) {
        try {
          const filesData = JSON.parse(splitFilesMeta);
          const blobsData = JSON.parse(splitFilesBlobs);

          // Process each split file
          Promise.all(
            blobsData.map((base64Data: string, index: number) => {
            ...
            .then((splitFiles) => {
              setUploadedFiles(splitFiles);
              // Clean up sessionStorage
              sessionStorage.removeItem("splitFilesMeta");
              sessionStorage.removeItem("splitFilesBlobs");
            })
            .catch((error) => {
          ...
          return; // Exit early if we found split files
        } catch (error) {
          console.error("Error parsing split files data:", error);
          sessionStorage.removeItem("splitFilesMeta");
          sessionStorage.removeItem("splitFilesBlobs");
        }
      }

      // Check for single file data (from other tools)
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
              ...
              <div className="xl:col-span-3 space-y-6">
                ...
                >
                  ...
                  {/* Merge Button */}
                  {uploadedFiles.length >= 2 && !mergeProgress.isProcessing && (
                    <motion.div
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      ...
                      >
                        🚀 Merge {uploadedFiles.length} PDFs into One
                      </button>
