...
export default async function AdminPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-10">
      <h1 className="text-3xl font-bold mb-6 text-orange-700">Suggestions & Bug Reports</h1>
          ...
          suggestions.map((s) => (
            <div key={s.id} className="border rounded-lg p-4 bg-white shadow">
              <div className="flex justify-between items-center mb-2">
                <span className="font-semibold text-orange-700">{s.name || 'Anonymous'}</span>
              ...
              {s.email && (
                <a href={`mailto:${s.email}`} className="text-xs text-blue-600 underline">{s.email}</a>
              )}
            </div>
          ))
        )}
